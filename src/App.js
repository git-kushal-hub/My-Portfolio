...
const App = () => {
  ...
  const handleTts = async (textToSpeak) => {
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${TTS_MODEL}:generateContent?key=${apiKey}`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          contents: [{ parts: [{ text: `Say in a warm, sincere, trustworthy tone: ${textToSpeak}` }] }],
          generationConfig: {
  ...
  const generateRoadmapAction = async () => {
    ...
    const systemPrompt = "You are Kushal Poudel, a Financial Partner. Generate a supportive, 4-step 'Growth Path' for a client's dream. Focus on providing stability and real care. Format as valid JSON with properties: step1, step2, step3, step4.";
    try {
  ...
  const handleAiQuery = async () => {
    ...
    setIsAiLoading(true);
    const systemPrompt = `You are Kushal Poudel. You are a professional accountant, but more importantly, you are a partner who cares deeply about people. Respond as a friend would. Keep it short, human, and sincere. Use 'I' and 'you'.`;
    try {
  ...
  const expertise = [
    { icon: Handshake, title: "Personalized Taxation", desc: "I handle the complex codes because I want you to focus on your family and business, not the paperwork." },
    { icon: ShieldCheck, title: "Auditing & Protection", desc: "My job is to provide the transparency that gives you total peace of mind every night." },
    { icon: Receipt, title: "Caring Payroll", desc: "I ensure your team is taken care of precisely, because their stability is the heart of your success." },
    { icon: TrendingUp, title: "Security Advisor", desc: "We'll build a path toward long-term financial security that respects your hard work." },
    { icon: Briefcase, title: "Your Growth Partner", desc: "Think of me as your in-house advocate, making sure you're ready for every new opportunity." },
    { icon: Landmark, title: "Administrative Ease", desc: "I'll take the 'boring' stuff off your plate so you can rediscover why you started your business." }
  ];
  ...
  return (
    <div className="bg-[#020617] text-white min-h-screen font-sans selection:bg-emerald-500/30 overflow-x-hidden" style={{ colorScheme: 'dark' }}>
          ...
          >
            ...
            >
              <div className="flex items-center gap-4 p-6 border-b border-white/5">
                ...
                <input 
                  autoFocus 
                  placeholder="How can I help you feel more secure today?" 
                  className="bg-transparent w-full outline-none text-xl text-white placeholder:text-gray-700 font-light" 
                  value={aiQuery}
                  onChange={(e) => setAiQuery(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && handleAiQuery()}
                />
                <button onClick={handleAiQuery} className="p-2 bg-emerald-600 rounded-xl hover:bg-emerald-500">
                ...
                {aiResponse ? (
                  <div className="mb-8">
                    <div className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                      <UserCheck size={12} className="text-emerald-500" /> A Message from Kushal
                    </div>
                    <div className="text-lg text-slate-300 leading-relaxed bg-white/5 p-6 rounded-2xl border border-white/5 italic">
                      {aiResponse}
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {[
                      { label: "Book a Coffee Chat", icon: Coffee, color: "text-emerald-400", sub: "Let's talk about your dreams" },
                      { label: "My Professional Story", icon: FileText, color: "text-amber-400", sub: "Why I care about this work" },
                      { label: "Client Portal", icon: Landmark, color: "text-sky-400", sub: "Secure access for partners" },
                      { label: "Reach Out Personally", icon: UserCheck, color: "text-emerald-500", sub: "Send me a direct note" }
                    ].map((item, i) => (
        ...
        <div className="max-w-[1400px] mx-auto px-8 flex justify-between items-center text-white">
          ...
          <div className="flex items-center gap-10">
            <div className="hidden lg:flex gap-10 text-[10px] font-black tracking-[0.3em] uppercase text-gray-400">
              {['How I Help', '✨ Growth Path', 'My Journey', 'Connect'].map((item) => (
                <a key={item} href={`#${item.toLowerCase().replace('✨ ', '').replace(' ', '-')}`} className="hover:text-emerald-400 transition-all">{item}</a>
              ))}
            </div>
            <button onClick={() => setIsCommandOpen(true)} className="px-8 py-3 rounded-full bg-emerald-600 text-white text-[11px] font-black uppercase tracking-widest hover:bg-emerald-500 transition-all">SAY HELLO (K)</button>
          </div>
        </div>
      </nav>

      <section className="relative pt-32 pb-20 px-8 min-h-screen flex flex-col justify-center z-10 max-w-[1400px] mx-auto text-left">
        <div className="grid lg:grid-cols-[1fr_400px] gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1 }}>
            <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 text-[10px] font-black uppercase tracking-[0.3em] mb-12">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div> Rooted in trust
            </div>
            <h1 className="text-7xl md:text-[8rem] lg:text-[10rem] font-black tracking-tighter leading-[0.8] mb-12 text-white uppercase">
              YOUR PEACE OF MIND <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 via-amber-500 to-yellow-600 italic">MY PASSION.</span>
            </h1>
            <div className="max-w-2xl mb-12">
              <p className="text-slate-300 text-xl md:text-2xl leading-relaxed font-light italic border-l-2 border-emerald-500 pl-6 mb-6">
                "I know how heavy financial stress can feel. I don't just see numbers; I see the effort and dreams behind them. Let me shoulder the burden of the spreadsheets so you can focus on what you love."
              </p>
              <button onClick={() => handleTts("I know how heavy financial stress can feel. I am Kushal Poudel, and I am here to help shoulder that burden. Let's build your future together.")} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:text-white transition-colors">
                {isTtsLoading ? <Loader2 size={14} className="animate-spin" /> : <Volume2 size={14} />} 
                {isTtsLoading ? "One moment..." : "✨ Listen to a sincere message"}
              </button>
            </div>
            <div className="flex gap-6">
              <a href="mailto:mail@kushalpoudel.com" className="p-6 rounded-3xl bg-white/5 border border-white/10 text-gray-400 hover:text-emerald-400 transition-all"><Mail size={28} /></a>
              <a href="#" className="p-6 rounded-3xl bg-white/5 border border-white/10 text-gray-400 hover:text-emerald-400 transition-all"><Linkedin size={28} /></a>
            </div>
          </motion.div>
      ...
      <section id="growth-path" className="py-40 px-8 max-w-[1400px] mx-auto scroll-mt-20 text-left">
        <div className="bg-slate-900/40 border border-emerald-500/20 rounded-[3rem] p-8 md:p-16 relative overflow-hidden">
          ...
          <div className="relative z-10 grid lg:grid-cols-[1fr_2fr] gap-16">
            <div>
              ...
              <h3 className="text-4xl md:text-6xl font-black mb-8 text-white">✨ Your Path Forward.</h3>
              <p className="text-slate-400 mb-10 leading-relaxed italic">Let's look at your biggest worry together. Tell me what you're working toward, and we'll outline a supportive path that keeps you protected.</p>
              <div className="flex flex-col gap-4">
                <input type="text" placeholder="What are you dreaming of building?" className="bg-white/5 border border-white/10 rounded-2xl px-6 py-4 outline-none focus:border-emerald-500 transition-all" value={roadmapGoal} onChange={(e) => setRoadmapGoal(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && generateRoadmapAction()} />
            ...
            <div className="min-h-[400px] flex items-center justify-center bg-black/40 rounded-3xl border border-white/5 p-8 relative">
              {!generatedRoadmap && !isRoadmapLoading && <div className="text-slate-600 uppercase tracking-widest font-black text-sm italic">Tell me your dream above...</div>}
              {isRoadmapLoading && <div className="flex flex-col items-center gap-4 text-emerald-500 font-black uppercase tracking-widest text-xs animate-pulse"><Loader2 size={48} className="animate-spin" /> Thinking...</div>}
              {generatedRoadmap && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full space-y-6">
                  {Object.values(generatedRoadmap).map((step, idx) => (
                    <div key={idx} className="flex gap-6 items-start group">
                      ...
                      <div><div className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">Phase {idx+1}</div><p className="text-slate-300 text-lg font-light leading-snug">{step}</p></div>
                    </div>
                  ))}
                </motion.div>
              )}
      ...
      <section id="how-i-help" className="py-40 px-8 max-w-[1400px] mx-auto text-center">
        <h2 className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.5em] mb-4">Core Support</h2>
        <h3 className="text-4xl md:text-6xl font-black text-white italic mb-20">Built on real connection.</h3>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {expertise.map((skill, i) => (
            <motion.div key={i} className="p-10 rounded-[2.5rem] bg-slate-900/40 border border-white/5 group hover:border-emerald-500/50 transition-all shadow-2xl text-left">
              <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 flex items-center justify-center mb-8 group-hover:bg-emerald-500 group-hover:text-black transition-all"><skill.icon size={32} /></div>
      ...
      <footer id="connect" className="pt-20 pb-20 px-8 max-w-[1400px] mx-auto text-center">
        <div className="rounded-[4rem] bg-emerald-700 p-16 md:p-32 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6 relative z-10">
            <a href="mailto:mail@kushalpoudel.com" className="px-14 py-7 bg-white text-black rounded-3xl font-black text-xl hover:shadow-[0_0_50px_rgba(255,255,255,0.4)] transition-all uppercase flex items-center justify-center gap-4">Send me a note <Mail size={24} /></a>
          </div>
        </div>
        <div className="mt-32 pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-12 text-[10px] font-black text-slate-500 uppercase tracking-[0.5em]">
          <div>© 2024 KUSHAL POUDEL — BUILT FOR YOUR PEACE OF MIND</div>
          <div className="flex gap-10"><a href="#" className="hover:text-emerald-400 transition-all uppercase">LinkedIn</a><a href="mailto:mail@kushalpoudel.com" className="hover:text-emerald-400 transition-all uppercase">Mail</a></div>
        </div>
      </footer>
    </div>
  );
