...
// For Vercel, ensure REACT_APP_GEMINI_API_KEY is set in your Environment Variables.
const getApiKey = () => {
  try {
    // This safety check prevents "process is not defined" errors in the browser
...
const App = () => {
  ...
  const generateRoadmapAction = async () => {
    ...
    try {
      ...
      console.error(e);
      setGeneratedRoadmap({
        step1: "Connection to AI Strategist currently offline.",
        step2: "I'd still love to help you plan this journey.",
        step3: "Please reach out to me personally at mail@kushalpoudel.com.",
        step4: "We can build this path together."
      });
    } finally {
  ...
  return (
    <div className="bg-[#020617] text-white min-h-screen font-sans selection:bg-emerald-500/30 overflow-x-hidden" style={{ colorScheme: 'dark' }}>
          ...
          >
            ...
            >
              <div className="flex items-center gap-4 p-6 border-b border-white/5">
                ...
                <input 
                  autoFocus 
                  placeholder="How can I help you feel secure today?" 
              ...
              <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-emerald-900 text-left">
                {aiResponse ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {[
                      { label: "Book a Coffee Chat", icon: Coffee, color: "text-emerald-400", sub: "Let's talk about your goals" },
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1 }}>
            ...
            <div className="max-w-2xl mb-12">
              <p className="text-slate-300 text-xl md:text-2xl leading-relaxed font-light italic border-l-2 border-emerald-500 pl-6 mb-6">
                "I know how heavy the weight of financial stress can feel. I don't just see numbers on a page; I see the effort and dreams behind them. Let me shoulder the burden of the spreadsheets so you can build your legacy."
              </p>
              <button 
                onClick={() => handleTts("I know how heavy financial stress can feel. I am Kushal Poudel, and I am here to help. Let's build your future together.")}
              ...
              >
                ...
                {isTtsLoading ? "Thinking..." : "✨ Listen to a short message"}
              </button>
            </div>
          ...
          <div className="relative hidden lg:block">
            ...
            <div className="relative aspect-[3/4] rounded-[3rem] overflow-hidden border border-white/10 group shadow-2xl">
              ...
              <div className="absolute bottom-10 left-10 z-20 text-left">
                ...
                <div className="text-2xl font-black text-white uppercase">KUSHAL POUDEL</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="your-strategy" className="py-40 px-8 max-w-[1400px] mx-auto scroll-mt-20 text-left">
        <div className="bg-slate-900/40 border border-emerald-500/20 rounded-[3rem] p-8 md:p-16 relative overflow-hidden shadow-inner">
          ...
          <div className="relative z-10 grid lg:grid-cols-[1fr_2fr] gap-16">
            <div>
              ...
              <p className="text-slate-400 mb-10 leading-relaxed italic">Let's look at your biggest challenge together. Tell me what you're working toward, and my AI strategist will help us outline a supportive path forward.</p>
              <div className="flex flex-col gap-4">
                ...
                />
                <button onClick={generateRoadmapAction} disabled={isRoadmapLoading || !roadmapGoal} className="bg-emerald-600 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-emerald-500 transition-all shadow-lg active:scale-95">
                  {isRoadmapLoading ? <Loader2 className="animate-spin" size={20} /> : <Sparkles size={20} />} Outline My Path
                </button>
              </div>
            </div>

            <div className="min-h-[400px] flex items-center justify-center bg-black/40 rounded-3xl border border-white/5 p-8 relative shadow-2xl">
              {!generatedRoadmap && !isRoadmapLoading && <div className="text-slate-600 uppercase tracking-widest font-black text-sm italic opacity-50">Tell me your goal above...</div>}
              ...
              {generatedRoadmap && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full space-y-6">
                  {Object.values(generatedRoadmap).map((step, idx) => (
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} key={idx} className="flex gap-6 items-start group">
                      <div className="text-6xl font-black text-emerald-950 group-hover:text-emerald-500/20 transition-all leading-none italic select-none">{idx+1}</div>
                      <div>
                        <div className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">Phase {idx+1}</div>
      ...
      <section id="how-i-help" className="py-40 px-8 max-w-[1400px] mx-auto text-center">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {expertise.map((skill, i) => (
            <motion.div key={i} className="p-10 rounded-[2.5rem] bg-slate-900/40 border border-white/5 group hover:border-emerald-500/50 transition-all shadow-2xl text-left hover:bg-slate-900/60">
              ...
              <h3 className="text-2xl font-black mb-4 text-white uppercase">{skill.title}</h3>
              <p className="text-slate-400 text-sm leading-relaxed font-light">{skill.desc}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      {/* Trajectory */}
      <section id="my-journey" className="py-40 px-8 max-w-[1400px] mx-auto border-t border-white/5 text-left">
        <h2 className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.5em] mb-24">My Professional Journey</h2>
        <div className="space-y-16">
          {[
            ...
            <div key={i} className="grid md:grid-cols-[1fr_2fr] gap-8 items-start pb-16 border-b border-white/5 group">
              <div className="text-4xl font-black text-white/10 group-hover:text-amber-500/50 transition-colors italic select-none">{item.year}</div>
              <div>
                <h3 className="text-3xl font-black mb-2 text-white uppercase tracking-tight">{item.role}</h3>
                <div className="text-emerald-500 font-bold uppercase tracking-widest text-xs mb-4">{item.company}</div>
                <p className="text-slate-400 max-w-xl font-light leading-relaxed">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <footer id="connect" className="pt-20 pb-20 px-8 max-w-[1400px] mx-auto text-center">
        <div className="rounded-[4rem] bg-emerald-700 p-16 md:p-32 relative overflow-hidden shadow-2xl">
          ...
          <h2 className="text-6xl md:text-[8rem] font-black tracking-tighter mb-12 relative z-10 text-white leading-none italic uppercase">I'M IN THIS <br />WITH YOU.</h2>
          <p className="text-emerald-100 text-xl md:text-3xl font-light mb-16 max-w-3xl mx-auto leading-relaxed relative z-10 italic">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6 relative z-10">
            <a href="mailto:mail@kushalpoudel.com" className="px-14 py-7 bg-white text-black rounded-3xl font-black text-xl hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all uppercase flex items-center justify-center gap-3">
              Let's Connect <Mail size={24} />
            </a>
          </div>
        </div>
        <div className="mt-32 pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-12 text-[10px] font-black text-slate-500 uppercase tracking-[0.5em]">
          <div className="flex items-center gap-3"><div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div> Fiscal Strategy Online</div>
          <div className="text-white font-bold tracking-widest">© 2024 KUSHAL POUDEL — BUILT FOR YOUR PEACE OF MIND</div>
          <div className="flex gap-10">
            <a href="#" className="hover:text-emerald-400 transition-all uppercase">LinkedIn</a>
            <a href="mailto:mail@kushalpoudel.com" className="hover:text-emerald-400 transition-all uppercase">Mail</a>
          </div>
        </div>
      </footer>
    </div>
